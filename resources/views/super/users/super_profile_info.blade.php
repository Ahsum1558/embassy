                        ...
                        @if($superData->gender == 1)
                          ...
                          @endif
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-9"><span><input type="text" name="address" class="form-control d-inline-block inline_setup" value="{{ $superData->address }}"></span>
                                </div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-3">
                                ...
                                <div class="col-9"><input type="text" name="zipcode" class="form-control d-inline-block inline_setup" value="{{ $superData->zipcode }}">
                                </div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-3">
                                ...
                                @foreach($all_country as $country)
                                  <option value="{{ $country->id }}" {{ $superData->countryId == $country->id ? 'selected' : '' }}>{{ $country->countryname }}</option>
                                @endforeach
                                </select>
                                </div>
                            </div>
                            <div class="row mb-2">
                                ...
                                @foreach($all_division as $division)
                                  <option value="{{ $division->id }}" {{ $superData->divisionId == $division->id ? 'selected' : '' }}>{{ $division->divisionname }}</option>
                                @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="row mb-2">
                                ...
                                @foreach($all_district as $district)
                                  <option value="{{ $district->id }}" {{ $superData->districtId == $district->id ? 'selected' : '' }}>{{ $district->districtname }}</option>
                                @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="row mb-2">
                                ...
                                @foreach($all_city as $city)
                                    <option value="{{ $city->id }}" {{ $superData->cityId == $city->id ? 'selected' : '' }}>{{ $city->cityname }}</option>
                                @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="row mb-2">
                                ...
                                @foreach($all_upzila as $upzila)
                                    <option value="{{ $upzila->id }}" {{ $superData->policestationId == $upzila->id ? 'selected' : '' }}>{{ $upzila->policestationname }}</option>
                                @endforeach
                                    </select>
                                </div>
                            </div>
...
<script>
    $(document).ready(function(){
        $('#select').change(function() {
            ...
            if(countryId){
                $('#overlay .loader').show();
                $.get("{{ route('super.profile.get') }}", {country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
        ...
        $('#single-select').change(function() {
            ...
            if(divisionId && countryId){
                $('#overlay .loader').show();
                $.get("{{ route('super.profile.getDistrict') }}", {division_id:divisionId, country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
        ...
        $('#id_label_single').change(function() {
            ...
            if(districtId && divisionId && countryId){
                $('#overlay .loader').show();
                $.get("{{ route('super.profile.getCity') }}", {district_id:districtId, division_id:divisionId, country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
        ...
        $('#id_label_single').change(function() {
            ...
            if(districtId && divisionId && countryId){
                $('#overlay .loader').show();
                $.get("{{ route('super.profile.getUpzila') }}", {district_id:districtId, division_id:divisionId, country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
