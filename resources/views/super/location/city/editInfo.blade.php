    ...
    <div class="col-12">
        <div class="card card_line">
            <div class="card-header card_headline">
                <h4 class="card-title headline">Update Info of {{ $city_data_info[0]->cityname }}</h4>
            </div>
            <div class="card-body">
@include('super.includes.alert')
                <form action="{{ route('super.city.updateInfo', ['id'=>$city_data_info[0]->id]) }}" class="form-group" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                    <!--Tab slider End-->
                    <div class="col">
                            ...
                            <div class="new-arrival-content pr">
                                ...
                                <div class="row mb-2">
                                    ...
                                    @foreach($all_division as $division)
                                      <option value="{{ $division->id }}" {{ $city_data_info[0]->divisionId == $division->id ? 'selected' : '' }}>{{ $division->divisionname }}</option>
                                    @endforeach
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-9">
                                      ...
                                      <option value="{{ $district->id }}" {{ $city_data_info[0]->districtId == $district->id ? 'selected' : '' }}>{{ $district->districtname }}</option>
                                    @endforeach
                                ...
                                @if($city_data_info[0]->status == 1)
                                          <option selected="selected" value="1">Active</option>
                                          <option value="0">Inactive</option>
                                  @elseif($city_data_info[0]->status == 0)
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-9 mybtn">
                                        ...
                                        <a class="btn submitbtn mb-2 form-control inline_setup text-uppercase pull-right" href="{{ route('super.city.show', ['id'=>$city_data_info[0]->id]) }}">Back</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    $(document).ready(function(){
        $('#select').change(function() {
            ...
            if(countryId){
                $('#overlay .loader').show();
                $.get("{{ route('super.city.get') }}", {country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
        ...
        $('#single-select').change(function() {
            ...
            if(divisionId && countryId){
                $('#overlay .loader').show();
                $.get("{{ route('super.city.getDistrict') }}", {division_id:divisionId, country_id:countryId}, function(data){
                    $('#overlay .loader').hide();
                    console.log(data);
