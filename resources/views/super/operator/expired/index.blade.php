...
<div class="row page-titles mx-0">
    ...
    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active"><a href="javascript:void(0)">Expired Licenses</a></li>
        </ol>
    </div>
</div>
    ...
    <div class="col-xl-12 col-lg-12">
        <div class="card card_line">
            <div class="card-header card_headline">
               <h4 class="card-title headline">All Expired License</h4>
            </div>
            <div class="card-body">
              <h4 class="mb-4 basic_headline">Expired Licenses</h4>
              <div class="table-responsive">
                  <table id="example" class="display" style="min-width: 700px">
                      ...
                      <tbody>
                        @php
                          $i=1;
                        @endphp
                      @foreach($all_expireduser as $super_user)
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td><a href="{{ route('super.operator.show', ['id'=>$super_user->id]) }}">{{ $super_user->title }}</a></td>
                            ...
                            <td>
                              ...
                              <a class="edit_option bg-success" href="#approvedId{{ $super_user->id }}" data-toggle="modal"><i class="fas fa-check"></i><span>Approve Operator</span></a>
                              <a class="edit_option bg-opacity-100" href="#pendingId{{ $super_user->id }}" data-toggle="modal"><i class="fas fa-hourglass-end"></i><span>Pending Operator</span></a>
                              <a class="edit_option bg-danger" href="#disableId{{ $super_user->id }}" data-toggle="modal"><i class="fas fa-ban"></i><span>Disable Operator</span></a>
                            @if($super_user->status == 'active')
                ...
                @include('super.operator.expired.users_modal')
                @include('super.operator.expired.users_inactive')
                @include('super.operator.expired.users_active')
                @include('super.operator.expired.users_approve')
                @include('super.operator.expired.users_pending')
                @include('super.operator.expired.users_disable')
                        </tr>
                @endforeach
                      </tbody>
                  </table>
